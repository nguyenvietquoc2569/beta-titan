...
addRuleToIAuthUrlConfig('/api/v1/multimedia/reject', {
...
export default withParamCheck(withSession(withUserParse(withAuthMiddle(async (req, res) => {
  ...
  const nLog = new LogActionModel({
    ...
    des: {
      vi: `${_loginSession.user.username} đã từ chối yêu cầu duyệt media ${media._id.toString()}`,
      en: `${_loginSession.user.username} rejected to approve media ${media._id.toString()}`,
    },
