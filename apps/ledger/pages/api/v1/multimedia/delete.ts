...
export default withParamCheck(withSession(withUserParse(withAuthMiddle(async (req, res) => {
  ...
  media.status = EMultimediaStatus.ACHIEVED
  ...
  const nLog = new LogActionModel({
    ...
    des: {
      vi: `${_loginSession.user.username} đã Xoá media ${media._id.toString()}`,
      en: `${_loginSession.user.username} deleted media ${media._id.toString()} for approval`,
    },
