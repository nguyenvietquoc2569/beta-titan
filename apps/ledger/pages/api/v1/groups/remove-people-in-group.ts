...
addRuleToIAuthUrlConfig('/api/v1/groups/remove-people-in-group', {
...
export default withParamCheck(withSession(withUserParse(withAuthMiddle(async (req, res) => {
  ...
  const nLog = new LogActionModel({
    ...
    center: _loginSession.workingCenter,
    des: {
      vi: `${_loginSession.user.username} đã xoá ${staff.username} vào group ${group.name} ở center ${_loginSession.workingCenter.name}`,
      en: `${_loginSession.user.username} removed ${staff.username} into group ${group.name} at center ${_loginSession.workingCenter.name}`,
    },
  ...
  staff.groups = staff.groups.filter((id) => {
    return id.toString()!==new ObjectId(_idGroup).toString()
  })
